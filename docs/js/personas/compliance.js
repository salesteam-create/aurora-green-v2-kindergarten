      ...
      ${items.map(({vessel, component}) => `
        <div class="card p-5 card-hover cursor-pointer" data-vid="${vessel.id}" data-cid="${component.id}">
          <div class="flex items-center justify-between mb-2">
            <div class="text-xs text-slate-400">${vessel.name} · Reg. no ${vessel.imo}</div>
...
window.ComplianceViews['cert-engine'] = function (root) {
  ...
  root.innerHTML = `
    <div class="flex items-end justify-between mb-6">
      <div>
        <div class="text-xs text-slate-400">${v.name} · Reg. no ${v.imo}</div>
      ...
      ${showSubmit ? `<button id="submit-all" ${anyReady ? '' : 'disabled'} class="px-4 py-2 rounded-lg font-semibold ${anyReady ? 'bg-teal-400 text-slate-900 hover:bg-teal-300' : 'bg-slate-700 text-slate-500 cursor-not-allowed'} ${anyReady && window.demoState._submitJustUnlocked ? 'submit-pulse' : ''}">Submit for Authority Review</button>` : ''}
    </div>
      ...
      onComplete: (filename, findings) => {
        window.updateState(state => {
          ...
          state.recentActivity.unshift({ at: new Date().toISOString(), persona: 'compliance', session: true,
            text: `AI pre-validated ${cr.name}. Ready for authority review.` });
        });
  ...
  submitBtn && submitBtn.addEventListener('click', () => {
    ...
    window.updateState(state => {
      ...
      if (submissions.length) {
        state.recentActivity.unshift({ at: new Date().toISOString(), persona: 'compliance', session: true,
          text: `Submitted ${submissions.length} item${submissions.length>1?'s':''} for authority review.` });
      }
    });
    window.toast(`${submissions.length} submission${submissions.length>1?'s':''} sent for authority review`);
  });
};
...
function reviewerFeedbackCard(feedback) {
  ...
  const notes = feedback.reviewerNotes || 'Reviewer requested changes — see the authority report.';
  return `
    ...
    <div class="card p-6 space-y-5">
      ...
      ` : cert.status === 'pending-review' ? `
        ...
        <div class="bg-sky-500/10 border border-sky-500/30 rounded-lg p-4">
          <div class="flex items-center gap-2 text-sky-300 font-medium"><i data-lucide="clock" class="w-4 h-4"></i> Pre-validated · Awaiting authority review</div>
...
window.ComplianceViews.submissions = function (root) {
  ...
  root.innerHTML = `
    <div class="mb-6">
      ...
      <div class="text-slate-400 text-sm mt-1">Items sent for authority review.</div>
    </div>
    <div class="card p-5 mb-5">
      <div class="font-medium mb-3">Pending authority review (${subs.length})</div>
